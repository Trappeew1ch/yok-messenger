/**
 * YOK Seed Phrase Recovery System
 * 
 * Generates cryptographically secure 12-word recovery phrases
 * using a BIP39-inspired wordlist. The phrase is never stored —
 * only a salted PBKDF2 hash is saved to the database.
 * 
 * Security:
 * - 12 words from 2048-word list = ~132 bits of entropy
 * - PBKDF2-SHA256 with 100,000 iterations for hashing
 * - Per-user salt prevents rainbow table attacks
 * - Phrase is shown only once during registration
 */
...
/**
 * Generate a cryptographically secure random index into the wordlist.
 */
...
export function generateSeedPhrase(): string[] {
  ...
  for (let i = 0; i < PHRASE_LENGTH; i++) {
    phrase.push(WORDLIST[secureRandomIndex()]);
  }
  return phrase;
}

/**
 * Normalize a seed phrase for consistent hashing.
 * Lowercases, trims, and collapses whitespace.
 */
export function normalizeSeedPhrase(phrase: string | string[]): string {
  if (Array.isArray(phrase)) {
    return phrase.map(w => w.toLowerCase().trim()).join(' ');
  }
  return phrase.toLowerCase().trim().replace(/\s+/g, ' ');
}

/**
 * Validate that a phrase contains only valid words from the wordlist.
 * Returns an array of invalid words (empty if all valid).
 */
...
/**
 * Hash a seed phrase with a per-user salt using PBKDF2-SHA256.
 * The result is stored in the database — the raw phrase is NEVER stored.
 * 
 * @param phrase - The seed phrase (string or array)
 * @param userId - The user's UUID (used as part of the salt)
 * @returns Base64-encoded hash
 */
export async function hashSeedPhrase(
...
/**
 * Verify a seed phrase against a stored hash.
 * 
 * @param phrase - The user-entered seed phrase
...
export async function verifySeedPhrase(
  ...
  const computedHash = await hashSeedPhrase(phrase, userId);
